$(function() {
		$('#sayfalar > li > ul').before('<span class="closez" title="Sil"> - Sayfayý sil</span>');
...
function sayfaSil(id,delli){
	...
	$.ajax({
			  ...
			  success:(function(data){
					delli.remove();
					$('#pages').load(wbtrt + 'menu/pages',function(){$("form select.styled").select_unskin();$("form select.styled").select_skin();});
					$('.text').val('');
					$( "#dialog-message" ).dialog( "close" );
...
function ekle(webroot) {
	...
	$.ajax({
		  type: 'POST',
		  url: webroot + 'menu/add',
		  data: $('#yenisayfa').serialize(),
		  success:(function(data){
				$('#pages').load(webroot + 'menu/pages',function(){$("form select.styled").select_unskin();$("form select.styled").select_skin();});
				$('#altsayfa').show();
				if (data != '') {
					...
					$('#sayfalar > li:last > ul').before('<span class="closez" title="Sil"> - Sayfayý sil</span>');
