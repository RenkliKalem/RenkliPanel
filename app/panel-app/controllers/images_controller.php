...
class ImagesController extends AppController {
	...
	var $name = 'Images';
	var $components = array('Uploader.Uploader','PImage');
	function index($model = Null,$id = Null) {
		...
		$links = Array(
				...
				'service' => Array('controller'=>'services','link' => 'services', 'views' => 'Hizmetimiz', 'viewp' => 'Hizmetlere','model' => 'Service'),
				'sector' => Array('controller'=>'sectors','link' => 'sectors', 'views' => 'Sektörler', 'viewp' => 'Sektörlere','model' => 'Sector'),
				'reference' => Array('controller'=>'references','link' => 'references', 'views' => 'Referans', 'viewp' => 'Referanslara','model' => 'Reference'),
				'slider' => Array('controller'=>'sliders','link' => 'sliders', 'views' => 'Slider', 'viewp' => 'Slidera','model' => 'Slider'),
				'feature' => Array('controller'=>'features','link' => 'features', 'views' => 'Ürün Özellikleri', 'viewp' => 'Ürün Özelliklerine','model' => 'Feature')
					);
		...
		if (!empty($this->data)) {
    		...
    		if ($data = $this->Uploader->upload('fileName', array('overwrite' => true, 'name' => $fileName))) {
				...
				if (is_array($filetypes) && isset($filetypes['ImageSize']) && !empty($filetypes['ImageSize'])) {
	...
	function delete($model = null,$id = Null,$imageId = Null) {
		if (!$imageId) {
	...
	function setmain($model = null,$id = Null,$imageId = Null) {
		if (!$imageId) {
			$this->Session->setFlash(__('<p>Yanlış Fotoğraf</p>', true),'default',array('class' => 'message info'));
			$this->redirect(array('action'=>'index/'. $model . '/' . $id));
		}
